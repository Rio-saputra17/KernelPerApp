...
class AppDetectionService : Service() {
    ...
    companion object {
        ...
        // Default profile — bisa diupdate dari ViewModel
        var defaultGovernor = ""
        var defaultMinFreq = ""
        var defaultMaxFreq = ""
    ...
    // Simpan state sistem saat pertama boot sebagai fallback
    private suspend fun saveSystemDefaults() {
        try {
            if (defaultGovernor.isBlank()) {
                defaultGovernor = readNode("/sys/devices/system/cpu/cpufreq/policy0/scaling_governor")
                    ?: readNode("/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor") ?: "schedutil"
            }
            if (defaultGpuGovernor.isBlank()) {
                defaultGpuGovernor = readNode("/sys/class/kgsl/kgsl-3d0/devfreq/governor") ?: ""
            }
        ...
        while (scope.isActive) {
            try {
                ...
                if (pkg.isNotBlank() && pkg != lastPackage) {
                    val profile = repository.getProfile(pkg)
                    val hasProfile = profile?.isEnabled == true

                    when {
                        hasProfile -> {
                            lastWasProfileApp = true
                            lastPackage = pkg
                            currentForegroundApp = pkg
                            repository.applyProfile(pkg)
                        }
                        lastWasProfileApp -> {
                            lastWasProfileApp = false
                            lastPackage = pkg
                            currentForegroundApp = pkg
                            restoreDefaults()
                        }
    ...
    private suspend fun restoreDefaults() {
        try {
            ...
            if (defaultGovernor.isNotBlank())
                for (i in 0 until n) Shell.cmd("echo '$defaultGovernor' > /sys/devices/system/cpu/cpu$i/cpufreq/scaling_governor").exec()
            if (defaultMinFreq.isNotBlank())
                for (i in 0 until n) Shell.cmd("echo '$defaultMinFreq' > /sys/devices/system/cpu/cpu$i/cpufreq/scaling_min_freq").exec()
            if (defaultMaxFreq.isNotBlank())
                for (i in 0 until n) Shell.cmd("echo '$defaultMaxFreq' > /sys/devices/system/cpu/cpu$i/cpufreq/scaling_max_freq").exec()
            if (defaultGpuGovernor.isNotBlank())
                Shell.cmd("echo '$defaultGpuGovernor' > /sys/class/kgsl/kgsl-3d0/devfreq/governor").exec()
            if (defaultScheduler.isNotBlank())
                for (b in listOf("sda", "sdb", "mmcblk0"))
                    Shell.cmd("echo '$defaultScheduler' > /sys/block/$b/queue/scheduler").exec()
        } catch (e: Exception) { }
    }
    ...
    private fun getForegroundApp(): String {
        ...
        for (cmd in cmds) {
            ...
            if (r.isSuccess) for (line in r.out) {
                val p = extractPkg(line)
    ...
    private fun getCpuCount(): Int =
        Shell.cmd("ls /sys/devices/system/cpu/ | grep -c 'cpu[0-9]'").exec()
